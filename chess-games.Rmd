...
I have chosen to train the model on 80% of the data set, which leaves 20% for testing.

```{r}
set.seed(123)
sample <- createDataPartition(games$white_rating, p=.8, list=F)
training <- games[sample,]
testing <- games[-sample,]

lm1 <- train(white_rating~., data=training, method = "lm")
```