...
gbm <- train(white_rating ~ ., data=training, method="gbm", trControl=tc)
...
model_metrics <- add_mm(glm_mod, "Generalised linear model")
model_metrics <- add_mm(knn_mod, "K-nearest neighbours")
model_metrics <- add_mm(rlm_mod,  "Robust linear model")
