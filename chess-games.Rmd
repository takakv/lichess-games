...
cart_mod <- train(white_rating ~ ., data=training, method="rpart")

#glm_mod <- train(white_rating ~ ., data=training, method="randomGLM")
ext_mod <- train(white_rating ~ ., data=training, method="xgbLinear")
brnn_mod <- train(white_rating ~ ., data=training, method="brnn")
...
$$\frac{1}{12271512}*\frac{1}{8}=98172096$$

### Odds of matching 6

#### Without Viking Number

1 in 12,271,512

#### With Viking Number (guess 5 + Viking Number):

$$\frac{48!}{5!(48-5)!}=\frac{48!}{5!*43!}=1712304$$

Probability of the non-guessed number being 8 or smaller:
$$\frac{9-1}{48}=\frac{8}{48}=\frac{1}{6}$$



### Odds of matching 5 + Viking Number

$$\frac{48!}{5!(48-5)!}=\frac{48!}{5!*43!}=1 712 304$$
$$\frac{1}{1 712 304}*\frac{1}{8}=$$
